...
export function PaginationTestScreen() {
	// Generate 25 paragraphs of ~520 characters each = ~13,000 characters total.
	const paragraphs = Array.from({
		length: 25,
	}).map((_, i) => ({
		id: `large-para-${i}`,
		text: `This is paragraph number ${i + 1}. It is part of a very large document designed to test the pagination capabilities of the MCP Browser Kit. We need a lot of characters to exceed the 5,000 character limit of a single page, so we are repeating this text many times. This ensures that the document is split across multiple pages, allowing us to test that fetching subsequent pages retrieves the correct content, that the cache is properly invalidated, and that all metadata fields like totalPages, pageNumber, hasNextPage, and nextPageNumber are correct.`,
	}));

	// Generate 220 buttons to test element pagination (>2 pages of 100 elements each).
